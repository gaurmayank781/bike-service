...
public class FeedBackModel {
	...
	public Integer nextPK() throws DatabaseException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement("SELECT MAX(ID) FROM B_FeedBack");
	...
	public long add(FeedBackBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement("INSERT INTO B_feedBack VALUES(?,?,?,?,?,?,?,?)");
			pstmt.setInt(1, pk);
	...
	public void delete(FeedBackBean bean) throws ApplicationException {
		...
		try {
			conn = JDBCDataSource.getConnection();
			conn.setAutoCommit(false); // Begin transaction
			PreparedStatement pstmt = conn.prepareStatement("DELETE FROM B_FeedBack WHERE ID=?");
	...
	public FeedBackBean findByPK(long pk) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("SELECT * FROM B_FeedBack WHERE ID=?");
	...
	public List search(FeedBackBean bean) throws ApplicationException {
	...
	public List search(FeedBackBean bean, int pageNo, int pageSize) throws ApplicationException {
		log.debug("Model search Started");
		StringBuffer sql = new StringBuffer("SELECT * FROM B_FeedBack WHERE 1=1");
